...
Page({
  data: {
    riddles: util.shuffle(dataProvider.riddles),
    tip: '',
    question: '',
    answer: [],
    answer_tip: [],
  ...
  // generateAnswerTip() {
  //   let num = this.data.num;
  //   let tip = util.contact(this.data.riddles[num].answer.split(""), util.generateWord(6))
  //   let arr = [];
  ...
  generateAnswerTip() {
    ...
    let tips = [];
    let num = this.data.num;
    let length = this.data.riddles.length;
    for (let i = 0; i < length; i++) {
      if (i != num) {
        arr = arr.concat(this.data.riddles[i].answer);
      }
    }
    let reg = /[\u4e00-\u9fa5]/g;
    arr = arr.join('').match(reg);
    arr = util.getArrayItems(arr, 5);
    let answer = util.shuffle(this.data.riddles[num].answer.split(""));
  ...
  onShareAppMessage: function () {
    ...
    if (!this.data.isCorrect) {
      ...
      if (num > 0) {
        title = '这个谜语太有趣了，我已经过了第' + num + '关，看看你能过几关！';
      } else {
        title = '敢不敢在睡觉前玩这个游戏？';
      } 
      this.setData({
    ...
    return {
      ...
      // desc: '',
      path: '/pages/riddle/index?id=' + this.data.num,
  ...
  next() {
    let num = this.data.num;
    if (num < this.data.riddles.length - 1) {
      num++;
      this.generateQue(num);
      this.setData({
        status: false
  ...
  generateQue(i) {
    ...
    this.setData({
      question: this.data.riddles[i].question
    })
    this.setData({
      tip: this.data.riddles[i].answer
    })
    let arr = this.data.riddles[i].answer.split("");
    this.setData({
  ...
  tapTip(e) {
    ...
    if (this.data.riddles[num].answer == answer) {
      ...
      if (this.data.riddles[num].answer.length == this.data.errorCount) {
